	...
	openInbox(function (err, box) {
		...
		imap.on("mail", (mail) => {
			...
			var f = imap.seq.fetch(box.messages.total + ":*", {
				bodies: [
					"HEADER.FIELDS (FROM TO SUBJECT DATE BODY)",
					"TEXT",
				],
			});
					...
					simpleParser(stream, async (err, parsed) => {
						...
						if (headers.size) {
							...
							if (parsed.html && statusRead) {
								const [match] = parsed.html.match(
									REGEXS.HTML
								);
								let [description] = parsed.html.match(
									REGEXS.DESCRIPTION
								);
								let [amount] = parsed.html.match(
									REGEXS.AMOUNT
								);
								...
								if (match) {
									...
									if (
										...
										description =
											description.replace(
												/Thanh toan QR |QR - (.*) Chuyen tien |QR - /g,
												""
											);
