...
const { APP_NAME, MAIL_CONTACT, HOST } = process.env;
...
function send(to, data) {
	...
	mailer.send(
		data.subject,
		pug.renderFile("./modules/mail.pug", {
			...data,
			APP_NAME,
			MAIL_CONTACT,
		}),
...
module.exports = {
	sendNeedApproved: (data) => {
		send(MAIL_CONTACT, {
			subject: `Có yêu cầu cần duyệt từ ${data.fullname}`,
			content: `<h1>Có đăng ký mới cần duyệt.</h1>
                    <a href="${HOST}admin/registrations">Duyệt tại đây</a>`,
		});
	},
	...
	approveRegister: (to, { name, exams }) => {
		...
		exams.sort((a, b) => {
			...
			return (
				new Date(a.date.setHours(aH, aM, 0, 0)) -
				new Date(b.date.setHours(bH, bM, 0, 0))
			);
		});
